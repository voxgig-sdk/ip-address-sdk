...
## Install
```bash
go get github.com/voxgig-sdk/ip-address-sdk/go@latest
```

The Go module proxy resolves the version from the `go/vX.Y.Z` GitHub
release tag — see [Releases](https://github.com/voxgig-sdk/ip-address-sdk/releases) for the available versions.
...
```bash
go mod edit -replace github.com/voxgig-sdk/ip-address-sdk/go=../ip-address-sdk/go
```
...
bulkqueryip, err := client.BulkQueryIP(nil).Load(
    map[string]any{"id": "test01"}, nil,
)
if err != nil {
    panic(err)
}
fmt.Println(bulkqueryip) // the loaded mock data
```
...
```go
func NewIpAddressSDK(options map[string]any) *IpAddressSDK
```
...
```go
func TestSDK(testopts map[string]any, sdkopts map[string]any) *IpAddressSDK
```

Creates a test-mode client with mock transport. Both arguments may be `nil`.

### IpAddressSDK methods
...
### Entity interface (IpAddressEntity)

All entities implement the `IpAddressEntity` interface.
...
slice):
    ...
    bulkqueryip, err := client.BulkQueryIP(nil).Load(map[string]any{"id": "example_id"}, nil)
    if err != nil { /* handle */ }
    // bulkqueryip is the loaded record
...
```go
get_current_ip, err := client.GetCurrentIp(nil).Load(map[string]any{"id": "get_current_ip_id"}, nil)
if err != nil {
    panic(err)
}
...
```
github.com/voxgig-sdk/ip-address-sdk/go/
├── ip-address.go        # Root package — type aliases and constructors
...
The root package (`github.com/voxgig-sdk/ip-address-sdk/go`) re-exports everything needed
...
bulkqueryip.Load(map[string]any{"id": "example_id"}, nil)

// bulkqueryip.Data() now returns the loaded bulkqueryip data
// bulkqueryip.Match() returns the last match criteria
```
