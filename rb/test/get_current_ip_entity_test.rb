...
class GetCurrentIpEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = get_current_ip_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "get_current_ip." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set IPADDRESS_TEST_GET_CURRENT_IP_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    get_current_ip_ref01_data = nil
    if get_current_ip_ref01_data_raw.length > 0
...
def get_current_ip_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "get_current_ip", "GetCurrentIpTestData.json")
  ...
  entid_env_raw = ENV["IPADDRESS_TEST_GET_CURRENT_IP_ENTID"]
