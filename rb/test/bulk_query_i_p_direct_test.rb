...
class BulkQueryIPDirectTest < Minitest::Test
  def test_direct_list_bulk_query_i_p
    setup = bulk_query_i_p_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-bulk_query_i_p", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
    if setup[:live]
      ["bulk_query_i_p01"].each do |_live_key|
...
def bulk_query_i_p_direct_setup(mockres)
