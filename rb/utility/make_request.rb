...
module IpAddressUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = IpAddressResponse.new({})
    ...
    if fetch_err
      ...
      response = IpAddressResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = IpAddressResponse.new(fetched)
    else
