...
class GetIpIntelligenceEntity
  ...
  def make
    opts = @_entopts.dup
    GetIpIntelligenceEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = IpAddressHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [GetIpIntelligence, Hash] the current GetIpIntelligence data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = IpAddressHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of GetIpIntelligence fields)
  def match_get
  ...
  #
  # @param reqmatch [GetIpIntelligenceLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.GetIpIntelligence.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [GetIpIntelligence, Hash] the loaded GetIpIntelligence; raises IpAddressError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = IpAddressHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
