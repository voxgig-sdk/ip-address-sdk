...
// IpAddress SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class IpAddressMakeError
{
    public static function call(?IpAddressContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new IpAddressContext([], null);
        }
        $op = $ctx->op ?? new IpAddressOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new IpAddressResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof IpAddressError) ? $err->msg : (string)$err;
        $msg = "IpAddressSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new IpAddressError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof IpAddressError) {
