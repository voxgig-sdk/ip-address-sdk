...
class FtCtrl extends IpAddressControl
{
...
class FtClient
{
    ...
    private IpAddressUtility $utility;

    public function __construct(IpAddressUtility $utility, array $options)
    {
    ...
    public function get_utility(): IpAddressUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (IpAddressContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public IpAddressContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (IpAddressContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = IpAddressConfig::make_feature($name);
    ...
    private static function buildUrl(IpAddressSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof IpAddressError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new IpAddressResponse($fetched);
        }
    ...
    private function fail(IpAddressContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (IpAddressContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
