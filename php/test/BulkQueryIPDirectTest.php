...
class BulkQueryIPDirectTest extends TestCase
{
    public function test_direct_list_bulk_query_i_p(): void
    {
        $setup = bulk_query_i_p_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-bulk_query_i_p", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
        if ($setup["live"]) {
            foreach (["bulk_query_i_p01"] as $_liveKey) {
