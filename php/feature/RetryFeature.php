...
class IpAddressRetryFeature extends IpAddressBaseFeature
{
    ...
    public function init(IpAddressContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (IpAddressContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(IpAddressContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(IpAddressContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
