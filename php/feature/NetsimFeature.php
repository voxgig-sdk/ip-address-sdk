...
class IpAddressNetsimFeature extends IpAddressBaseFeature
{
    ...
    private function _simulate(IpAddressContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(IpAddressContext $ctx, array $applied): void
    {
        $client = $this->client;
