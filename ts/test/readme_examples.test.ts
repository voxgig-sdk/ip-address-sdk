...
//     (\`new IpAddressSDK(...)\` / \`IpAddressSDK.test(...)\` become
//     \`IpAddressSDK.test({ entity: ... })\`), transpiled to JS, and run
...
// rewrite real client construction (\`new IpAddressSDK(...)\` and any
// \`IpAddressSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+IpAddressSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/IpAddressSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
