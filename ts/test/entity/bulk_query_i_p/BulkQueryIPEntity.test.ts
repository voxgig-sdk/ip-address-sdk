...
describe('BulkQueryIPEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'bulk_query_i_p.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set IP_ADDRESS_TEST_BULK_QUERY_I_P_ENTID JSON to run live')
      return
    }
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['IP_ADDRESS_TEST_BULK_QUERY_I_P_ENTID']
