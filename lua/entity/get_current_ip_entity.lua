...
local GetCurrentIpEntity = {}
GetCurrentIpEntity.__index = GetCurrentIpEntity


function GetCurrentIpEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GetCurrentIpEntity)
  self._name = "get_current_ip"
  self._client = client
...
function GetCurrentIpEntity:get_name()
  return self._name
end


function GetCurrentIpEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GetCurrentIpEntity.new(self._client, opts)
end


function GetCurrentIpEntity:data_set(args)
  if args ~= nil then
...
function GetCurrentIpEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GetCurrentIpEntity:match_set(args)
  if args ~= nil then
...
function GetCurrentIpEntity:match_get()
...
function GetCurrentIpEntity:stream(action, args, callopts)
...
function GetCurrentIpEntity:_run_op(ctx, post_done)
  local utility = self._utility
