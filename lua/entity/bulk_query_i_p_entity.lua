...
local BulkQueryIPEntity = {}
BulkQueryIPEntity.__index = BulkQueryIPEntity


function BulkQueryIPEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, BulkQueryIPEntity)
  self._name = "bulk_query_i_p"
  self._client = client
...
function BulkQueryIPEntity:get_name()
  return self._name
end


function BulkQueryIPEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return BulkQueryIPEntity.new(self._client, opts)
end


function BulkQueryIPEntity:data_set(args)
  if args ~= nil then
...
function BulkQueryIPEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function BulkQueryIPEntity:match_set(args)
  if args ~= nil then
...
function BulkQueryIPEntity:match_get()
...
function BulkQueryIPEntity:stream(action, args, callopts)
...
function BulkQueryIPEntity:_run_op(ctx, post_done)
  local utility = self._utility
