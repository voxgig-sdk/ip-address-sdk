...
describe("BulkQueryIPDirect", function()
  it("should direct-list-bulk_query_i_p", function()
    local setup = bulk_query_i_p_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-bulk_query_i_p", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"bulk_query_i_p01"}) do
...
function bulk_query_i_p_direct_setup(mockres)
