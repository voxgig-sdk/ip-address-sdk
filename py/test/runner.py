...
class IpAddressTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            IpAddressTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = IpAddressTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = IpAddressTestRunner.getenv("IPADDRESS_TEST_LIVE")
        override = IpAddressTestRunner.getenv("IPADDRESS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = IpAddressTestRunner.getenv(key)
        ...
        explain = IpAddressTestRunner.getenv("IPADDRESS_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["IPADDRESS_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if IpAddressTestRunner._test_control is not None:
            return IpAddressTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                IpAddressTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            IpAddressTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = IpAddressTestRunner.load_test_control()
...
def load_env_local():
    IpAddressTestRunner.load_env_local()


def env_override(m):
    return IpAddressTestRunner.env_override(m)


def entity_list_to_data(lst):
    return IpAddressTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return IpAddressTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return IpAddressTestRunner.load_test_control()


def live_delay_ms():
    return IpAddressTestRunner.live_delay_ms()
