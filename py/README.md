# IpAddress Python SDK

The Python SDK for the IpAddress API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.BulkQueryIP(None).list(None, None)
if err:
    raise Exception(err)
...
```python
client = IpAddressSDK.test(testopts, sdkopts)
```
...
```
py/
├── ipaddress_sdk.py         -- Main SDK module
...
The main module (`ipaddress_sdk`) exports the SDK class.
