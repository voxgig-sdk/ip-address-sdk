...
class BulkQueryIPEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return BulkQueryIPEntity(self._client, opts)
    ...
    def match_get(self) -> BulkQueryIP:
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[BulkQueryIP]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.BulkQueryIP().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
