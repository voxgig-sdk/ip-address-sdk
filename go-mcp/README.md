...
[MCP](https://modelcontextprotocol.io) server exposing the IpAddress SDK as
two agent tools — `ip-address_list` and `ip-address_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ip-address-mcp)
make build
...
claude mcp add --scope user ip-address \
  -- /absolute/path/to/ip-address-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./ip-address-mcp -transport http -addr :8080
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ip-address-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user ip-address \
     -- "$PWD"/dist/*/ip-address-mcp -transport stdio
   ```
...
| Tool | Args | Returns |
|------|------|---------|
| `ip-address_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `ip-address_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `IP_ADDRESS_APIKEY` | API key sent with every request. |
| `IP_ADDRESS_BASE` | Optional override of the API base URL. |
...
```sh
./ip-address-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"ip-address_load","arguments":{"entity":"get_current_ip","query":{"id":1}}}}'
```
