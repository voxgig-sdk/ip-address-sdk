...
MCP server exposing the IpAddress SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `ip-address_list` | `entity`, optional `query` map | First page of records as JSON |
| `ip-address_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `ip-address_list` and
`ip-address_load` tools become available in new sessions.
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"ip-address_load","arguments":{"entity":"bulk_query_i_p","query":{"id":1}}}}'
```
