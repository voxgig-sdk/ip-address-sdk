// Command IpAddress-mcp is an MCP server exposing the IpAddress
// SDK as MCP tools.
//
// Two tools are registered:
//
//	IpAddress_list  — list records of an entity
//	IpAddress_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: IP_ADDRESS_APIKEY carries the API key and
	// IP_ADDRESS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("IP_ADDRESS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("IP_ADDRESS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewIpAddressSDK(opts)
