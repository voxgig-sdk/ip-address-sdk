...
AQL-driven command-line client **and** interactive REPL for the IpAddress
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ip-address-cli)
make build
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ip-address-cli
   ```
...
Run with no arguments to open a REPL (prompt `ip-address>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an AQL atom (e.g. `bulk_query_i_p`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/ip-address-cli`. |
...
(`./ip-address-cli <expr>`) and interactively (the REPL), and expressions compose the
